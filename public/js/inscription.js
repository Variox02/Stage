...
function validate(first_name, name, email, password, confirm, cgu, country, birth_date, address, telephone) {
    let valid = true;['prenom', 'nom', 'email', 'password', 'confirm', 'cgu', 'country', 'birth_date', 'address', 'telephone'].forEach(clearError)
    ...
    if(!telephone) { 
        showError ('telephone', 'Champ requis.'); valid = false 
...
document.getElementById('form-inscription').addEventListener('submit', async (e) => {
    ...
    if (!validate(first_name, name, email, password, confirm, cgu, country, birth_date, address, telephone)) return
    ...
    let emailDisponible
    try {
        const checkRes = await fetch(`${API_URL}/api/verifMail`, {
            method: 'POST',
    ...
    const response = await fetch(`${API_URL}/api/inscription`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ first_name, name, email, password, newsletter, country, birth_date, address, telephone })
    })
